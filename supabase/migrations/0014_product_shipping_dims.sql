-- Per-product shipping parameters (weight + package dimensions).
--
-- USPS prices a parcel from its weight and box size. These columns let the
-- operator enter each product's shipping weight (ounces) and dimensions
-- (inches) in the admin editor. The checkout rating service (Phase 2b) reads
-- them to build a combined package and call the USPS Prices API; when a
-- product has no dimensions, the USPS default box from the Integrations panel
-- is used as a fallback.
--
-- All four are nullable — products without weight fall back to the default
-- box. The save_product_full RPC is updated here so admin saves persist the
-- fields atomically alongside the other product columns.

alter table products
  add column if not exists weight_oz  numeric(8,2) null,
  add column if not exists length_in  numeric(8,2) null,
  add column if not exists width_in   numeric(8,2) null,
  add column if not exists height_in  numeric(8,2) null;

-- Non-negative sanity checks (nulls allowed).
alter table products drop constraint if exists products_weight_oz_nonneg;
alter table products add constraint products_weight_oz_nonneg
  check (weight_oz is null or weight_oz >= 0);
alter table products drop constraint if exists products_length_in_nonneg;
alter table products add constraint products_length_in_nonneg
  check (length_in is null or length_in >= 0);
alter table products drop constraint if exists products_width_in_nonneg;
alter table products add constraint products_width_in_nonneg
  check (width_in is null or width_in >= 0);
alter table products drop constraint if exists products_height_in_nonneg;
alter table products add constraint products_height_in_nonneg
  check (height_in is null or height_in >= 0);

-- Replace save_product_full to persist the shipping columns. Body is identical
-- to 0012 except for the four added columns in the product upsert.
...
declare
  ...
  v_ig_url        text := nullif(v_product ->> 'instagram_url', '');
  v_weight_oz     numeric := nullif(v_product ->> 'weight_oz', '')::numeric;
  v_length_in     numeric := nullif(v_product ->> 'length_in', '')::numeric;
  v_width_in      numeric := nullif(v_product ->> 'width_in', '')::numeric;
  v_height_in     numeric := nullif(v_product ->> 'height_in', '')::numeric;
begin
  if v_cat_slug is not null then
    ...
    if v_category_id is null then
      select id into v_category_id
        from categories
        where slug = v_cat_slug and parent_id is not null
          and parent_id in (select id from categories where audience = v_audience and parent_id is null)
        limit 1;
    end if;
  end if;
  ...
  if (v_product ->> 'id') is not null and (v_product ->> 'id') <> '' then
    ...
    update products set
      ...
      instagram_url  = v_ig_url,
      weight_oz      = v_weight_oz,
      length_in      = v_length_in,
      width_in       = v_width_in,
    ...
    insert into products (
      ...
      featured, best_seller, new_arrival, try_on_enabled, instagram_url,
