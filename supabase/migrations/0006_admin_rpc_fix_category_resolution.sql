-- save_product_full: fix category slug → id resolution.
--
-- The original RPC looked for the operator's chosen category as a row with
-- `parent_id IS NULL`. In our schema the only rows with `parent_id IS NULL`
-- are the audience roots ('women', 'men', 'kids'). The actual top-level
-- categories ('kurthis', 'sarees', etc.) are children of those audience
-- roots, so the lookup always failed and category_id/subcategory_id were
-- silently stored as NULL — meaning newly-saved products never appeared on
-- their category landing pages on the storefront.
--
-- Fix: resolve the audience root id first, then look up the operator's
-- chosen category as a child of that root, and the subcategory as a child
-- of the category. Three levels match the locked taxonomy.
...
begin
  -- Locate the audience root (e.g. the row with slug='women', parent_id=null).
  select id into v_audience_root_id
    from categories
    where parent_id is null and audience = v_audience
    limit 1;

  -- Resolve the operator's chosen top-level category as a child of the audience root.
  if v_cat_slug is not null and v_audience_root_id is not null then
    select id into v_category_id
      from categories
      where slug = v_cat_slug
        and parent_id = v_audience_root_id
        and audience = v_audience
      limit 1;
  end if;

  -- Resolve the operator's chosen subcategory as a child of the category.
