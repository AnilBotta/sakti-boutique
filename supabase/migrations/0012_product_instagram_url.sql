-- Per-product Instagram link.
--
-- Adds `instagram_url` to products so the operator can attach a reel /
-- post URL from the admin editor. When set, the PDP renders a
-- "Watch on Instagram" button under Add to Cart so shoppers can see
-- the piece in motion.
--
-- Nullable — most products won't have a reel; the PDP button is only
-- rendered when this column is populated. The save_product_full RPC
-- is updated in this migration so admin saves include the field
-- atomically alongside the other product columns.

alter table products
  add column if not exists instagram_url text null;

-- Light sanity check so we don't get random strings persisted. Only
-- validates the domain — full URL correctness is enforced in the
-- server action layer.
alter table products
  drop constraint if exists products_instagram_url_looks_like_url;
alter table products
  add constraint products_instagram_url_looks_like_url
  check (
    instagram_url is null
    or instagram_url ~* '^https?://([a-z0-9-]+\.)*instagram\.com/'
  );

-- Replace save_product_full to persist instagram_url. Body is identical
-- to 0006 except for the added column in the product upsert.
