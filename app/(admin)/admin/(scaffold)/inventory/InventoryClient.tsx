...
import type { VariantRow } from '@/lib/repositories/admin-inventory';
import { updateVariantStockAction } from '@/lib/actions/admin-inventory';
...
export function InventoryClient({ initialRows }: Props) {
  // Local mirror so optimistic updates feel instant.
  ...
  function persistStock(variantId: string, stock: number) {
    // Optimistic update
    setRows((prev) =>
      prev.map((r) => (r.variantId === variantId ? { ...r, stock } : r)),
    );
    startTransition(async () => {
      const res = await updateVariantStockAction(variantId, stock);
      if (!res.ok) {
        // Rollback on error
        setRows((prev) =>
          prev.map((r) =>
            r.variantId === variantId
              ? { ...r, stock: initialRows.find((ir) => ir.variantId === variantId)?.stock ?? r.stock }
              : r,
          ),
        );
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(filtered.map((r) => r.variantId)));
  };
  ...
  const bulkAdjust = (delta: number) => {
    selected.forEach((uid) => {
      const row = rows.find((r) => r.variantId === uid);
      if (row) persistStock(uid, Math.max(0, row.stock + delta));
    });
    setSelected(new Set());
  };
  const bulkSetZero = () => {
    if (!confirm(`Set stock to 0 for ${selected.size} variants?`)) return;
    selected.forEach((uid) => persistStock(uid, 0));
    setSelected(new Set());
  };
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-caption">
              ...
              <tbody className="divide-y divide-border-hairline">
                {filtered.map((r) => (
                  <tr key={r.variantId} className="hover:bg-bg-subtle">
                    ...
                    <td className="px-4 py-2 text-right">
                      <InlineStockEditor
                        value={r.stock}
                        onSave={(n) => persistStock(r.variantId, n)}
                      />
    ...
    return (
      <button
        ...
        className={`inline-flex h-8 min-w-[3rem] items-center justify-end px-2 nums-tabular tabular-nums hover:bg-bg-muted ${
          value === 0 ? 'text-accent-crimson' : 'text-text-primary'
        }`}
      >
        {value}
      </button>
